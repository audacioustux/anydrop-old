defmodule AnydropWeb.SendLive do
  ...
  def render(assigns) do
    ...
    <div class="w-full min-h-full relative">
      <div class="absolute opacity-20 w-full h-full -z-10" style="background-image: url(/images/bg/doodles.png)" />
      <%!-- <div
        class="bg-repeat row-start-1 col-start-1 -z-10"
      ...
      <div class="container mx-auto max-w-2xl px-4 justify-self-center">
        <div class="flex rounded-full h-16 w-16 bg-white shadow-2xl justify-center items-center mx-auto mt-16 p-3 transition-all duration-500 ring-1 ring-zinc-700 hover:ring-2 shadow-lg hover:shadow-md mb-16 overflow-hidden">
          <svg
            width="256"
            height="256"
            viewBox="0 0 256 256"
        ...
        <div class="border rounded inline p-2"> Sending to <%= @handle %> </div>
  ...
  def mount(%{"send_to" => handle}, _session, socket) do
  ...
  def handle_event("save", %{"drop" => drop_params}, socket) do
    case DropContext.create_drop(socket.assigns.handle, drop_params) do
      {:ok, _drop} ->
        {:noreply,
         socket
