    ...
    ~H"""
      ...
      >
        ...
        <p class="self-end">
          ...
          <span class="text-zinc-600 font-medium text-xs"><%= @created |> DateTime.to_date() %></span>
        </p>
        <button class="absolute bottom-0 w-full h-full" phx-click={show_modal("drop-modal-#{@id}")}></button>
        <.modal id={"drop-modal-#{@id}"}>
          <p class="leading-6
                    text-zinc-700 font-medium text-lg"
          >
            <%= @body |> sanitze_body() %>
